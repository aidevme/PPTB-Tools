...
function fixHtmlForPPTB(): Plugin {
    return {
        ...
        enforce: 'post',
        // Vite's dev server also runs transformIndexHtml on every request, not just `vite build`;
        // without this the dev server's own <script type="module"> entry point gets stripped too,
        // breaking ESM loading with "Cannot use import statement outside a module".
        apply: 'build',
