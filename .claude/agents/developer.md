...
- **Code layout:** keep framework-free domain logic (Dataverse queries, form XML parsing/mutation, manifest
  parsing) in `src/lib/`, with no React/DOM-UI imports beyond `DOMParser`/`XMLSerializer`; keep React state,
  orchestration, and rendering in `App.tsx` / `src/components/`. Match `tools/pcf2bpf/src/` as the reference
  layout for a new tool or a new feature in an existing one.
...
- **Domain invariants** (for Dataverse/BPF work specifically): BPFs are `workflow` records with
  ...
  `controlDescriptions/controlDescription[@forControl]/customControl[@formFactor]` in form XML.
